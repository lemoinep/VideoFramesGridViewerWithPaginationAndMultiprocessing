...
# Video Frames Grid Viewer with Pagination and multiprocessing + ZIP cache
...
class MovieGridViewer(QtWidgets.QWidget):
    def __init__(self, video_path, cols, rows, num_workers, max_pages=None, thumbnail_format="PNG"):
        ...
        self.thumbnail_format = thumbnail_format.upper()  # "PNG", "HEIF" ou "AVIF"
        ...
        # -- ZIP CACHE Paths
        self.zip_cache_path = os.path.join(self.cache_dir, "thumbnails.zip")
    ...
    def clear_zip_cache(self):
        if os.path.exists(self.zip_cache_path):
            os.remove(self.zip_cache_path)
        self.all_thumbnails_data = []
            ...
            if os.path.exists(self.zip_cache_path):
                try:
                    with zipfile.ZipFile(self.zip_cache_path, 'r') as zf:
                        file_name = f"thumb_{frame_no}.{ext}"
                    ...
                    print(f"Erreur lecture image {file_name} dans le zip: {e}")
            if not found:
    ...
    def handle_loaded(self, fut, idx):
        ...
        if img is not None:
            ...
            file_name = f"thumb_{frame_no}.{ext}"
            try:
                with zipfile.ZipFile(self.zip_cache_path, 'a', compression=zipfile.ZIP_DEFLATED) as zf:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Video Frames Grid Viewer with Pagination, multiprocessing and ZIP cache")
